"""
財務省「普通貿易統計」からLNG（HS 271111）の月次 CIF 価格（円/MT）を取得する。

データフロー:
  1. e-Stat のファイル一覧ページ（HTML）から stat_infid を収集（API キー不要）
  2. e-Stat のファイルダウンロードエンドポイントから CSV を取得
  3. CSV から HS 271111 の行を抽出し、月別の数量（MT）・金額（千円）を読み取る
...
HS_CODE     = "271111"  # 液化天然ガス（LNG）
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "lng_cif_monthly.csv")
...
# ─── Step 3: CSV から HS 271111 の月次データを抽出 ───────────────────────
def _parse_monthly(csv_text: str) -> dict[str, dict]:
    """
    CSV テキストを解析し、HS 271111 輸入の月次データを返す。
    ...
    for row in reader:
        ...
        if not hs.startswith(HS_CODE):                             # HS 271111
            continue
        ...
        for i in range(len(_MONTH_NAMES)):
            ...
            result[ym] = {"quantity_mt": qty, "value_1000yen": val}

    return result


# ─── Step 4: メイン関数 ──────────────────────────────────────────────────
def get_lng_cif_price(months: int = 12) -> list[dict]:
    """
    LNG（HS 271111）の月次 CIF 価格（円/MT）を返す。
    ...
    for year in range(start_year, current_year + 1):
        ...
        for infid in reversed(infids):
            ...
            if monthly:
                print(f"    {infid}: HS 271111 データ {len(monthly)} 件", file=sys.stderr)
...
def main() -> None:
    print("=== LNG CIF 価格取得 ===")
    print("データソース: 財務省 普通貿易統計 / HS 271111 / 輸入 / 月次\n")

    try:
        records = get_lng_cif_price(months=120)
    except RuntimeError as e:
    ...
    diff   = (f"  前月比: {latest['cif_yen_per_mt'] - prev['cif_yen_per_mt']:+,.0f} 円/MT"
              if prev else "")
    print(f"\n--- 最新値サマリー ---")
    print(f"  LNG CIF: {latest['cif_yen_per_mt']:>10,.0f} 円/MT  ({latest['year_month']}){diff}")
    print("\n完了")
